...
class Downsample(nnx.Module):
    def __init__(
        ...
        padding: Union[str, Tuple[int]] = "SAME",
        use_bias: bool = False,
        *,
        rngs: nnx.Rngs
    ) -> None:
        kernel_size = 1 if strides == 1 and dilation == 1 else kernel_size
        ...
        self.conv = nnx.Conv(
            ...
            kernel_dilation=(first_dilation, first_dilation),
            padding=padding,
            use_bias=use_bias,
            rngs=rngs,
        )
...
class Classifier(nnx.Module):
    ...
    def __init__(self, num_pooled_features: int, num_classes: int, *, rngs: nnx.Rngs) -> None:
...
class BasicBlock(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        use_aa = aa_layer is not None and (strides == 2 or first_dilation != dilation)

        self.conv1 = nnx.Conv(
            in_features=inplanes,
            out_features=first_planes,
            kernel_size=(3, 3),
            strides=1 if use_aa else strides,
            padding=(first_dilation, first_dilation),
            kernel_dilation=(first_dilation, first_dilation),
            use_bias=False,
            rngs=rngs
        )
        self.bn1 = norm_layer(num_features=first_planes, rngs=rngs)
        self.drop_block = drop_block(rngs=rngs) if drop_block is not None else nnx.identity
        self.act1 = act_layer
...
def downsample_avg(
    ...
    *,
    ...
    # first_dilation, padding kept for call-site compat with Downsample
    norm_layer = norm_layer or nnx.BatchNorm
    avg_stride = strides if dilation == 1 else 1
    need_pool = avg_stride > 1 or (dilation > 1 and strides > 1)

    class AvgDownsample(nnx.Module):
        def __init__(self):
            if need_pool:
                pool_stride = avg_stride if avg_stride > 1 else 1
                self.pool = lambda x: nnx.avg_pool(
                    x, window_shape=(2, 2), strides=(pool_stride, pool_stride),
                )
            else:
                self.pool = None
            self.conv = nnx.Conv(
...
def make_blocks(
    ...
    for stage_idx, (block_fn, planes, num_blocks, db) in enumerate(
        ...
        if stride != 1 or inplanes != planes * block_fn.expansion:
            ...
            downsample = downsample_avg(**down_kwargs) if avg_down else Downsample(**down_kwargs)
...
class ResNet(nnx.Module):
        ...
        *,
        ...
        if deep_stem:
            ...
            self.conv1 = nnx.Sequential(
                nnx.Conv(
                    in_features=in_chans, out_features=stem_chs[0],
                ...
                act_layer,
                nnx.Conv(
                    in_features=stem_chs[0], out_features=stem_chs[1],
                ...
                act_layer,
                nnx.Conv(
                    in_features=stem_chs[1], out_features=inplanes,
                    kernel_size=(3, 3), strides=(1, 1), padding=(1, 1),
                    use_bias=False, rngs=rngs,
                ),
            )
        else:
            self.conv1 = nnx.Conv(in_features=in_chans, out_features=inplanes, kernel_size=(7, 7), strides=(2, 2), padding=(3, 3), use_bias=False, rngs=rngs)
        ...
        if replace_stem_pool:
            self.maxpool = nnx.Sequential(
                nnx.Conv(
                    in_features=inplanes, out_features=inplanes,
                    kernel_size=(3, 3), strides=(2, 2), padding=(1, 1),
                    use_bias=False, rngs=rngs,
                ),
            ...
            if aa_layer is not None:
                self.maxpool = nnx.Sequential(
                    lambda x: nnx.max_pool(inputs=x, window_shape=(3, 3), strides=(1, 1), padding=((1, 1), (1, 1))),
                    AntiAliasingLayer(aa_layer, strides=(2, 2)),
                )
            else:
                self.maxpool = lambda x: nnx.max_pool(inputs=x, window_shape=(3, 3), strides=(2, 2), padding=((1, 1), (1, 1)))
        ...
        self.stage_modules = nnx.List([l for i, l in stage_modules])
    ...
    def forward_head(self, x: jax.Array, pre_logits: bool = False) -> jax.Array:
        x = self.global_pool(x)
        x = x.reshape(x.shape[0], -1)
        if pre_logits:
            return x
        if self.drop_rate > 0.:
            x = nnx.Dropout(rate=self.drop_rate, deterministic=True)(x)
        x = self.fc(x)
        return x
...
def _cfg(url: str = '', **kwargs) -> Dict[str, Any]:
    return {
        'url': url,
        'num_classes': 1000, 'input_size': (224, 224, 3), 'pool_size': (7, 7),
...
def _tcfg(url: str = '', **kwargs) -> Dict[str, Any]:
    return _cfg(url=url, **dict({
        'interpolation': 'bicubic', 'pool_size': (8, 8),
        'input_size': (256, 256, 3), 'crop_pct': 0.94,
        'test_input_size': (320, 320, 3), 'test_crop_pct': 1.0,
    }, **kwargs))


def _ttcfg(url: str = '', **kwargs) -> Dict[str, Any]:
    return _cfg(url=url, **dict({
        'interpolation': 'bicubic', 'pool_size': (7, 7),
        'input_size': (224, 224, 3), 'crop_pct': 0.95,
        'test_input_size': (288, 288, 3), 'test_crop_pct': 1.0,
    }, **kwargs))
...
def _r3cfg(url: str = '', **kwargs) -> Dict[str, Any]:
    return _cfg(url=url, **dict({
        'interpolation': 'bicubic', 'input_size': (160, 160, 3),
        'pool_size': (5, 5), 'crop_pct': 0.91,
        'test_input_size': (224, 224, 3), 'test_crop_pct': 0.95,
        'origin_url': 'https://github.com/Xrenya/JaxNN',
        'paper_ids': 'arXiv:2110.00476'
    }, **kwargs))
...
# ──────────────────────────────────────────────────────────────────────
# Default configs - all timm ResNet variants
...
default_cfgs = generate_default_cfgs({
    ...
    'resnet10t.c3_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(176, 176, 3), pool_size=(6, 6),
        first_conv='conv1.0',
    ),
    ...
    'resnet18.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet18.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet18.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'resnet18.fb_ssl_yfcc100m_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnet18.fb_swsl_ig1b_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnet18.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnet18.tv_in1k': _cfg(hf_hub_id='timm/'),
    'resnet18d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet34.a1_in1k': _rcfg(
        hf_hub_id='JaxNN/resnet34.a1_in1k',
        url='https://huggingface.co/JaxNN/resnet34.a1_in1k',
    ),
    'resnet34.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet34.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'resnet34.bt_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnet34.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnet34.tv_in1k': _cfg(hf_hub_id='timm/'),
    'resnet34d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),

    # ResNet (Bottleneck)
    'resnet26.bt_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnet26d.bt_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet26t.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.a1h_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'resnet50.b1k_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.b2k_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.bt_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnet50.c1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.c2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.d_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet50.ram_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnet50.am_in1k': _tcfg(hf_hub_id='timm/'),
    'resnet50.ra_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnet50.fb_ssl_yfcc100m_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnet50.fb_swsl_ig1b_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnet50.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnet50.tv_in1k': _cfg(hf_hub_id='timm/'),
    'resnet50.tv2_in1k': _cfg(
        hf_hub_id='timm/',
        input_size=(176, 176, 3), pool_size=(6, 6),
        test_input_size=(224, 224, 3), test_crop_pct=0.965,
        interpolation='bilinear', crop_pct=0.875,
    ),
    'resnet50c.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50d.a1_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50d.a2_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50d.a3_in1k': _r3cfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50d.ra4_e3600_r224_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50s.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet50t.untrained': _ttcfg(first_conv='conv1.0'),
    'resnet101.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet101.a1h_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet101.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet101.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    ...
    'resnet101.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnet101c.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet101d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet101s.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet152.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet152.a1h_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet152.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnet152.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'resnet152.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    ...
    'resnet152c.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet152d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet152s.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnet200.untrained': _ttcfg(),
    'resnet200d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),

    # Wide ResNet
    'wide_resnet50_2.racm_in1k': _ttcfg(hf_hub_id='timm/'),
    ...
    # ResNeXt
    'resnext50_32x4d.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'resnext50_32x4d.a1h_in1k': _rcfg(hf_hub_id='timm/'),
    'resnext50_32x4d.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'resnext50_32x4d.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'resnext50_32x4d.fb_ssl_yfcc100m_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext50_32x4d.fb_swsl_ig1b_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext50_32x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnext50_32x4d.ra_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnext50_32x4d.tv_in1k': _cfg(hf_hub_id='timm/'),
    ...
    'resnext50d_32x4d.bt_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnext101_32x4d.fb_ssl_yfcc100m_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext101_32x4d.fb_swsl_ig1b_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext101_32x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnext101_32x8d.fb_ssl_yfcc100m_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext101_32x8d.fb_swsl_ig1b_ft_in1k': _cfg(hf_hub_id='timm/'),
    'resnext101_32x8d.tv_in1k': _cfg(hf_hub_id='timm/'),
    ...
    'resnext101_64x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'resnext101_64x4d.tv_in1k': _cfg(hf_hub_id='timm/'),

    # Anti-aliased ResNets
    'resnetblur18.untrained': _cfg(),
    'resnetblur50.bt_in1k': _ttcfg(hf_hub_id='timm/'),
    'resnetblur50d.untrained': _ttcfg(first_conv='conv1.0'),
    'resnetblur101d.untrained': _ttcfg(first_conv='conv1.0'),
    'resnetaa34d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnetaa50.a1h_in1k': _rcfg(hf_hub_id='timm/'),
    'resnetaa50d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnetaa50d.d_in12k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0', num_classes=11821),
    'resnetaa50d.d_in12k_ft_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'resnetaa101d.sw_in12k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0', num_classes=11821),
    'resnetaa101d.sw_in12k_ft_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),

    # SE-ResNet
    'seresnet18.untrained': _cfg(),
    'seresnet34.untrained': _cfg(),
    'seresnet50.a1_in1k': _rcfg(hf_hub_id='timm/'),
    'seresnet50.a2_in1k': _rcfg(hf_hub_id='timm/'),
    'seresnet50.a3_in1k': _r3cfg(hf_hub_id='timm/'),
    'seresnet50.ra2_in1k': _ttcfg(hf_hub_id='timm/'),
    'seresnet50t.untrained': _ttcfg(first_conv='conv1.0'),
    'seresnet101.untrained': _ttcfg(),
    'seresnet152.untrained': _ttcfg(),
    'seresnet152d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    ...
    # SE-ResNeXt
    'seresnext26d_32x4d.bt_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'seresnext26t_32x4d.bt_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'seresnext50_32x4d.racm_in1k': _ttcfg(hf_hub_id='timm/'),
    'seresnext50_32x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'seresnext101_32x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'seresnext101_32x8d.ah_in1k': _rcfg(hf_hub_id='timm/'),
    'seresnext101d_32x8d.ah_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'seresnext101_64x4d.gluon_in1k': _gcfg(hf_hub_id='timm/'),
    'seresnextaa101d_32x8d.ah_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'seresnextaa101d_32x8d.sw_in12k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0', num_classes=11821),
    'seresnextaa101d_32x8d.sw_in12k_ft_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'seresnextaa101d_32x8d.sw_in12k_ft_in1k_288': _rcfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(288, 288, 3), pool_size=(9, 9),
        test_input_size=(320, 320, 3), crop_pct=1.0,
    ),
    'seresnextaa201d_32x8d.untrained': _ttcfg(first_conv='conv1.0'),
    'senet154.gluon_in1k': _gcfg(hf_hub_id='timm/', first_conv='conv1.0'),

    # ECA-ResNet
    'ecaresnet26t.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    ...
    'ecaresnet50t.a1_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'ecaresnet50t.a2_in1k': _rcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'ecaresnet50t.a3_in1k': _r3cfg(hf_hub_id='timm/', first_conv='conv1.0'),
    'ecaresnet50t.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    ...
    'ecaresnet269d.ra2_in1k': _ttcfg(hf_hub_id='timm/', first_conv='conv1.0'),
    ...
    'resnetrs50.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(160, 160, 3), pool_size=(5, 5), crop_pct=0.91,
        test_input_size=(224, 224, 3), interpolation='bicubic',
    ),
    'resnetrs101.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(192, 192, 3), pool_size=(6, 6), crop_pct=0.94,
        test_input_size=(288, 288, 3), interpolation='bicubic',
    ),
    'resnetrs152.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(256, 256, 3), pool_size=(8, 8), crop_pct=1.0,
        test_input_size=(320, 320, 3), interpolation='bicubic',
    ),
    ...
    'resnetrs270.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(256, 256, 3), pool_size=(8, 8), crop_pct=1.0,
        test_input_size=(352, 352, 3), interpolation='bicubic',
    ),
    'resnetrs350.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(288, 288, 3), pool_size=(9, 9), crop_pct=1.0,
        test_input_size=(384, 384, 3), interpolation='bicubic',
    ),
    'resnetrs420.tf_in1k': _cfg(
        hf_hub_id='timm/', first_conv='conv1.0',
        input_size=(320, 320, 3), pool_size=(10, 10), crop_pct=1.0,
        test_input_size=(416, 416, 3), interpolation='bicubic',
    ),
})
...
def resnetblur18(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=BasicBlock, layers=(2, 2, 2, 2), aa_layer=partial(nnx.avg_pool))
...
def resnetblur50(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=Bottleneck, layers=(3, 4, 6, 3), aa_layer=partial(nnx.avg_pool))
...
def resnetaa34d(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(
        block=BasicBlock, layers=(3, 4, 6, 3), stem_width=32, stem_type='deep', avg_down=True,
        aa_layer=partial(nnx.avg_pool),
    )
...
def seresnet269d(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=Bottleneck, layers=(3, 30, 48, 8), stem_width=32, stem_type='deep', avg_down=True)
...
def seresnextaa101d_32x8d(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(
        block=Bottleneck, layers=(3, 4, 23, 3), cardinality=32, base_width=8,
        stem_width=32, stem_type='deep', avg_down=True, aa_layer=partial(nnx.avg_pool),
    )
...
def seresnextaa201d_32x8d(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(
        ...
        stem_width=64, stem_type='deep', avg_down=True, aa_layer=partial(nnx.avg_pool),
    )
...
def ecaresnet50d_pruned(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=Bottleneck, layers=(3, 4, 6, 3), stem_width=32, stem_type='deep', avg_down=True)
    return _create_resnet('ecaresnet50d_pruned', pretrained, **dict(model_args, **kwargs))
...
def ecaresnetlight(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=Bottleneck, layers=(1, 1, 11, 3), stem_width=32, avg_down=True)
...
def ecaresnet101d_pruned(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(block=Bottleneck, layers=(3, 4, 23, 3), stem_width=32, stem_type='deep', avg_down=True)
    return _create_resnet('ecaresnet101d_pruned', pretrained, **dict(model_args, **kwargs))
...
def resnetrs50(pretrained: bool = False, **kwargs) -> ResNet:
    ...
    model_args = dict(
        ...
        replace_stem_pool=True, avg_down=True,
    )
